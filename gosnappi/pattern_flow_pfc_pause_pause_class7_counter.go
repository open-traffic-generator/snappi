...
type marshalPatternFlowPfcPausePauseClass7Counter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowPfcPausePauseClass7Counter to raw JSON text
...
func (m *marshalpatternFlowPfcPausePauseClass7Counter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowPfcPausePauseClass7Counter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 65535 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowPfcPausePauseClass7Counter.Count <= 65535 but Got %d", *obj.obj.Count))
		}
