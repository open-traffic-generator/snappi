...
type marshalPatternFlowIpv4PriorityRawCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowIpv4PriorityRawCounter to raw JSON text
...
func (m *marshalpatternFlowIpv4PriorityRawCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowIpv4PriorityRawCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 255 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowIpv4PriorityRawCounter.Count <= 255 but Got %d", *obj.obj.Count))
		}
