...
type marshalPatternFlowEthernetPauseControlOpCode interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowEthernetPauseControlOpCode to raw JSON text
...
func (m *marshalpatternFlowEthernetPauseControlOpCode) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowEthernetPauseControlOpCode) setDefault() {
	...
	var choice PatternFlowEthernetPauseControlOpCodeChoiceEnum

	if obj.obj.Value != nil {
		choices_set += 1
		choice = PatternFlowEthernetPauseControlOpCodeChoice.VALUE
	}

	if len(obj.obj.Values) > 0 {
		choices_set += 1
		choice = PatternFlowEthernetPauseControlOpCodeChoice.VALUES
	}

	if obj.obj.Increment != nil {
		choices_set += 1
		choice = PatternFlowEthernetPauseControlOpCodeChoice.INCREMENT
	}

	if obj.obj.Decrement != nil {
		choices_set += 1
		choice = PatternFlowEthernetPauseControlOpCodeChoice.DECREMENT
	}
	if choices_set == 0 {
		...
		if obj.obj.Choice != nil {
			if obj.Choice() != choice {
				obj.validationErrors = append(obj.validationErrors, "choice not matching with property in PatternFlowEthernetPauseControlOpCode")
			}
		} else {
			intVal := otg.PatternFlowEthernetPauseControlOpCode_Choice_Enum_value[string(choice)]
			enumValue := otg.PatternFlowEthernetPauseControlOpCode_Choice_Enum(intVal)
			obj.obj.Choice = &enumValue
		}
	}
