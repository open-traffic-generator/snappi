...
type marshalPatternFlowRsvpReserved interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowRsvpReserved to raw JSON text
...
func (m *marshalpatternFlowRsvpReserved) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowRsvpReserved) setDefault() {
	...
	var choice PatternFlowRsvpReservedChoiceEnum

	if obj.obj.Value != nil {
		choices_set += 1
		choice = PatternFlowRsvpReservedChoice.VALUE
	}

	if len(obj.obj.Values) > 0 {
		choices_set += 1
		choice = PatternFlowRsvpReservedChoice.VALUES
	}

	if obj.obj.Increment != nil {
		choices_set += 1
		choice = PatternFlowRsvpReservedChoice.INCREMENT
	}

	if obj.obj.Decrement != nil {
		choices_set += 1
		choice = PatternFlowRsvpReservedChoice.DECREMENT
	}
	if choices_set == 0 {
		...
		if obj.obj.Choice != nil {
			if obj.Choice() != choice {
				obj.validationErrors = append(obj.validationErrors, "choice not matching with property in PatternFlowRsvpReserved")
			}
		} else {
			intVal := otg.PatternFlowRsvpReserved_Choice_Enum_value[string(choice)]
			enumValue := otg.PatternFlowRsvpReserved_Choice_Enum(intVal)
			obj.obj.Choice = &enumValue
		}
	}
