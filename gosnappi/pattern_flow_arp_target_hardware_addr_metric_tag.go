...
type marshalPatternFlowArpTargetHardwareAddrMetricTag interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowArpTargetHardwareAddrMetricTag to raw JSON text
...
func (m *marshalpatternFlowArpTargetHardwareAddrMetricTag) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
