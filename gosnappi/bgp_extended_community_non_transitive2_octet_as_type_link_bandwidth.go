...
type marshalBgpExtendedCommunityNonTransitive2OctetAsTypeLinkBandwidth interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals BgpExtendedCommunityNonTransitive2OctetAsTypeLinkBandwidth to raw JSON text
...
func (m *marshalbgpExtendedCommunityNonTransitive2OctetAsTypeLinkBandwidth) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
