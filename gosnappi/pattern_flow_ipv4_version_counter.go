...
type marshalPatternFlowIpv4VersionCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowIpv4VersionCounter to raw JSON text
...
func (m *marshalpatternFlowIpv4VersionCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowIpv4VersionCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 15 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowIpv4VersionCounter.Count <= 15 but Got %d", *obj.obj.Count))
		}
