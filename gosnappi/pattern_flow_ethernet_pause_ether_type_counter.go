...
type marshalPatternFlowEthernetPauseEtherTypeCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowEthernetPauseEtherTypeCounter to raw JSON text
...
func (m *marshalpatternFlowEthernetPauseEtherTypeCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowEthernetPauseEtherTypeCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 65535 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowEthernetPauseEtherTypeCounter.Count <= 65535 but Got %d", *obj.obj.Count))
		}
