...
type configAppend struct {
	...
	resourcesHolder ConfigAppendConfigAppendResourceIter
}
...
type ConfigAppend interface {
	...
	setDefault()
	// Resources returns ConfigAppendConfigAppendResourceIterIter, set in ConfigAppend
	Resources() ConfigAppendConfigAppendResourceIter
	setNil()
}

// Configuration resources of various types to be appended.
// Resources returns a []ConfigAppendResource
func (obj *configAppend) Resources() ConfigAppendConfigAppendResourceIter {
	if len(obj.obj.Resources) == 0 {
		obj.obj.Resources = []*otg.ConfigAppendResource{}
	}
	if obj.resourcesHolder == nil {
		obj.resourcesHolder = newConfigAppendConfigAppendResourceIter(&obj.obj.Resources).setMsg(obj)
	}
	return obj.resourcesHolder
}
...
func (obj *configAppend) validateObj(vObj *validation, set_default bool) {
	...
	if len(obj.obj.Resources) != 0 {
		...
		if set_default {
			obj.Resources().clearHolderSlice()
			for _, item := range obj.obj.Resources {
				obj.Resources().appendHolderSlice(&configAppendResource{obj: item})
			}
		}
		for _, item := range obj.Resources().Items() {
