...
type marshalPatternFlowIcmpv6EchoSequenceNumberCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowIcmpv6EchoSequenceNumberCounter to raw JSON text
...
func (m *marshalpatternFlowIcmpv6EchoSequenceNumberCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowIcmpv6EchoSequenceNumberCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 65535 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowIcmpv6EchoSequenceNumberCounter.Count <= 65535 but Got %d", *obj.obj.Count))
		}
