...
// BmpServerState is the information learned by a specific BMP Server.
type BmpServerState interface {
	...
	// ClientRouterId returns string, set in BmpServerState.
	ClientRouterId() string
	// SetClientRouterId assigns string provided by user to BmpServerState
	SetClientRouterId(value string) BmpServerState
	// HasClientRouterId checks if ClientRouterId has been set in BmpServerState
	HasClientRouterId() bool
...
// The IPv4 Router ID of the BMP client to which this BMP Server is connected. Note that each BMP Server is connected to a single BMP client, though multiple BMP Servers can be configured on the  same IPv4 or IPv6 interface , each connected to a different BMP client.  Each BMP client can in turn report information about multiple BGP sessions on that router.
// ClientRouterId returns a string
func (obj *bmpServerState) ClientRouterId() string {

	return *obj.obj.ClientRouterId

}

// The IPv4 Router ID of the BMP client to which this BMP Server is connected. Note that each BMP Server is connected to a single BMP client, though multiple BMP Servers can be configured on the  same IPv4 or IPv6 interface , each connected to a different BMP client.  Each BMP client can in turn report information about multiple BGP sessions on that router.
// ClientRouterId returns a string
func (obj *bmpServerState) HasClientRouterId() bool {
	return obj.obj.ClientRouterId != nil
}

// The IPv4 Router ID of the BMP client to which this BMP Server is connected. Note that each BMP Server is connected to a single BMP client, though multiple BMP Servers can be configured on the  same IPv4 or IPv6 interface , each connected to a different BMP client.  Each BMP client can in turn report information about multiple BGP sessions on that router.
// SetClientRouterId sets the string value in the BmpServerState object
func (obj *bmpServerState) SetClientRouterId(value string) BmpServerState {

	obj.obj.ClientRouterId = &value
	return obj
}
...
func (obj *bmpServerState) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.ClientRouterId != nil {

		err := obj.validateIpv4(obj.ClientRouterId())
		if err != nil {
			vObj.validationErrors = append(vObj.validationErrors, fmt.Sprintf("%s %s", err.Error(), "on BmpServerState.ClientRouterId"))
		}
