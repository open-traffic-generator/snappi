...
type marshalPatternFlowPfcPauseControlOpCodeMetricTag interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowPfcPauseControlOpCodeMetricTag to raw JSON text
...
func (m *marshalpatternFlowPfcPauseControlOpCodeMetricTag) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
