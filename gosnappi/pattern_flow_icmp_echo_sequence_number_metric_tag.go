...
type marshalPatternFlowIcmpEchoSequenceNumberMetricTag interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowIcmpEchoSequenceNumberMetricTag to raw JSON text
...
func (m *marshalpatternFlowIcmpEchoSequenceNumberMetricTag) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
