...
type marshalPatternFlowArpTargetProtocolAddrMetricTag interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowArpTargetProtocolAddrMetricTag to raw JSON text
...
func (m *marshalpatternFlowArpTargetProtocolAddrMetricTag) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
