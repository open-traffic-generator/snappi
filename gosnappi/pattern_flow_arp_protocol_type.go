...
type marshalPatternFlowArpProtocolType interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowArpProtocolType to raw JSON text
...
func (m *marshalpatternFlowArpProtocolType) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowArpProtocolType) setDefault() {
	...
	var choice PatternFlowArpProtocolTypeChoiceEnum

	if obj.obj.Value != nil {
		choices_set += 1
		choice = PatternFlowArpProtocolTypeChoice.VALUE
	}

	if len(obj.obj.Values) > 0 {
		choices_set += 1
		choice = PatternFlowArpProtocolTypeChoice.VALUES
	}

	if obj.obj.Increment != nil {
		choices_set += 1
		choice = PatternFlowArpProtocolTypeChoice.INCREMENT
	}

	if obj.obj.Decrement != nil {
		choices_set += 1
		choice = PatternFlowArpProtocolTypeChoice.DECREMENT
	}
	if choices_set == 0 {
		...
		if obj.obj.Choice != nil {
			if obj.Choice() != choice {
				obj.validationErrors = append(obj.validationErrors, "choice not matching with property in PatternFlowArpProtocolType")
			}
		} else {
			intVal := otg.PatternFlowArpProtocolType_Choice_Enum_value[string(choice)]
			enumValue := otg.PatternFlowArpProtocolType_Choice_Enum(intVal)
			obj.obj.Choice = &enumValue
		}
	}
