...
type marshalPatternFlowIpv4OptionsCustomTypeOptionNumberCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowIpv4OptionsCustomTypeOptionNumberCounter to raw JSON text
...
func (m *marshalpatternFlowIpv4OptionsCustomTypeOptionNumberCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
