...
type marshalPatternFlowIcmpEchoSequenceNumberCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowIcmpEchoSequenceNumberCounter to raw JSON text
...
func (m *marshalpatternFlowIcmpEchoSequenceNumberCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
