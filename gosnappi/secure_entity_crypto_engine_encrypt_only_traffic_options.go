...
type marshalSecureEntityCryptoEngineEncryptOnlyTrafficOptions interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals SecureEntityCryptoEngineEncryptOnlyTrafficOptions to raw JSON text
...
func (m *marshalsecureEntityCryptoEngineEncryptOnlyTrafficOptions) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
