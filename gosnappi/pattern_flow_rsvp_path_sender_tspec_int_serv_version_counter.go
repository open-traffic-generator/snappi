...
type marshalPatternFlowRSVPPathSenderTspecIntServVersionCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowRSVPPathSenderTspecIntServVersionCounter to raw JSON text
...
func (m *marshalpatternFlowRSVPPathSenderTspecIntServVersionCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
