...
type marshalPatternFlowGtpExtensionExtensionLengthMetricTag interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowGtpExtensionExtensionLengthMetricTag to raw JSON text
...
func (m *marshalpatternFlowGtpExtensionExtensionLengthMetricTag) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
