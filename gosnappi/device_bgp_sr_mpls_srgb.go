...
// DeviceBgpSrMplsSrgb is this contains the propeties of Segment Routing Global Block (SRGB) Sub-Tlv (3) that is advertised under PREFIX_SID Tlv (40). Although it is implementation depended, however must be advertised in first Update message at least. SR Global Block (SRGB): the set of global segments in the SR domain. If a node participates in multiple SR domains, there is one SRGB for each SR domain.  In SR-MPLS, SRGB is a local property of a node and identifies the set of local labels reserved for global segments.  In SR-MPLS, using identical SRGBs on all nodes within the SR domain is strongly recommended. Reference: https://datatracker.ietf.org/doc/html/rfc8402
