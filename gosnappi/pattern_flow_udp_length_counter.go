...
type marshalPatternFlowUdpLengthCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowUdpLengthCounter to raw JSON text
...
func (m *marshalpatternFlowUdpLengthCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowUdpLengthCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 65535 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowUdpLengthCounter.Count <= 65535 but Got %d", *obj.obj.Count))
		}
