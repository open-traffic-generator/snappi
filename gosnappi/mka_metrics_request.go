...
type MkaMetricsRequest interface {
	...
	setDefault()
	// KayNames returns []string, set in MkaMetricsRequest.
	KayNames() []string
	// SetKayNames assigns []string provided by user to MkaMetricsRequest
	SetKayNames(value []string) MkaMetricsRequest
...
// The names of key agreement entities (KaYs) to return results for. An empty list will return results for all KaYs.
//
// x-constraint:
// - /components/schemas/Mka/properties/name
//
// KayNames returns a []string
func (obj *mkaMetricsRequest) KayNames() []string {
	if obj.obj.KayNames == nil {
		obj.obj.KayNames = make([]string, 0)
	}
	return obj.obj.KayNames
}
...
// - /components/schemas/Mka/properties/name
//
// SetKayNames sets the []string value in the MkaMetricsRequest object
func (obj *mkaMetricsRequest) SetKayNames(value []string) MkaMetricsRequest {

	if obj.obj.KayNames == nil {
		obj.obj.KayNames = make([]string, 0)
	}
	obj.obj.KayNames = value
...
// The list of column names that the returned result set will contain. If the list is empty then all columns will be returned except for any result_groups. The name of the KaY cannot be excluded.
