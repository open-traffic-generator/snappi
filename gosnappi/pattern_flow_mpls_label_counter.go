...
type marshalPatternFlowMplsLabelCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowMplsLabelCounter to raw JSON text
...
func (m *marshalpatternFlowMplsLabelCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowMplsLabelCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 1048575 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowMplsLabelCounter.Count <= 1048575 but Got %d", *obj.obj.Count))
		}
