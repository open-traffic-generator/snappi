...
type marshalPatternFlowArpHardwareLength interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowArpHardwareLength to raw JSON text
...
func (m *marshalpatternFlowArpHardwareLength) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowArpHardwareLength) setDefault() {
	...
	var choice PatternFlowArpHardwareLengthChoiceEnum

	if obj.obj.Value != nil {
		choices_set += 1
		choice = PatternFlowArpHardwareLengthChoice.VALUE
	}

	if len(obj.obj.Values) > 0 {
		choices_set += 1
		choice = PatternFlowArpHardwareLengthChoice.VALUES
	}

	if obj.obj.Increment != nil {
		choices_set += 1
		choice = PatternFlowArpHardwareLengthChoice.INCREMENT
	}

	if obj.obj.Decrement != nil {
		choices_set += 1
		choice = PatternFlowArpHardwareLengthChoice.DECREMENT
	}
	if choices_set == 0 {
		...
		if obj.obj.Choice != nil {
			if obj.Choice() != choice {
				obj.validationErrors = append(obj.validationErrors, "choice not matching with property in PatternFlowArpHardwareLength")
			}
		} else {
			intVal := otg.PatternFlowArpHardwareLength_Choice_Enum_value[string(choice)]
			enumValue := otg.PatternFlowArpHardwareLength_Choice_Enum(intVal)
			obj.obj.Choice = &enumValue
		}
	}
