...
type marshalPatternFlowArpSenderHardwareAddrCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowArpSenderHardwareAddrCounter to raw JSON text
...
func (m *marshalpatternFlowArpSenderHardwareAddrCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
