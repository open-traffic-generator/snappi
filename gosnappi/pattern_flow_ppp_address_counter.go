...
type marshalPatternFlowPppAddressCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowPppAddressCounter to raw JSON text
...
func (m *marshalpatternFlowPppAddressCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowPppAddressCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 255 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowPppAddressCounter.Count <= 255 but Got %d", *obj.obj.Count))
		}
