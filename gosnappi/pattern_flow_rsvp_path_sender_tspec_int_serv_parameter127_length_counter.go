...
type marshalPatternFlowRSVPPathSenderTspecIntServParameter127LengthCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowRSVPPathSenderTspecIntServParameter127LengthCounter to raw JSON text
...
func (m *marshalpatternFlowRSVPPathSenderTspecIntServParameter127LengthCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowRSVPPathSenderTspecIntServParameter127LengthCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 65535 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowRSVPPathSenderTspecIntServParameter127LengthCounter.Count <= 65535 but Got %d", *obj.obj.Count))
		}
