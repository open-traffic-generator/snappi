...
type marshalPatternFlowVxlanReserved1Counter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowVxlanReserved1Counter to raw JSON text
...
func (m *marshalpatternFlowVxlanReserved1Counter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowVxlanReserved1Counter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 255 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowVxlanReserved1Counter.Count <= 255 but Got %d", *obj.obj.Count))
		}
