...
type marshalPatternFlowGtpv2Spare1Counter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowGtpv2Spare1Counter to raw JSON text
...
func (m *marshalpatternFlowGtpv2Spare1Counter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowGtpv2Spare1Counter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 7 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowGtpv2Spare1Counter.Count <= 7 but Got %d", *obj.obj.Count))
		}
