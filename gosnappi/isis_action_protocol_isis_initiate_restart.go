...
// IsisActionProtocolIsisInitiateRestart is timers T1 and T2 are used both by a restarting router and a starting router. Timer T3 is used only by a restarting router.
...
// When the timer T2 expires or is canceled, indicating that synchroization of that level is complete and SPF for that level is run.
