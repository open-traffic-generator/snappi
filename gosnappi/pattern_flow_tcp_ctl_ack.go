...
type marshalPatternFlowTcpCtlAck interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowTcpCtlAck to raw JSON text
...
func (m *marshalpatternFlowTcpCtlAck) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowTcpCtlAck) setDefault() {
	...
	var choice PatternFlowTcpCtlAckChoiceEnum

	if obj.obj.Value != nil {
		choices_set += 1
		choice = PatternFlowTcpCtlAckChoice.VALUE
	}

	if len(obj.obj.Values) > 0 {
		choices_set += 1
		choice = PatternFlowTcpCtlAckChoice.VALUES
	}

	if obj.obj.Increment != nil {
		choices_set += 1
		choice = PatternFlowTcpCtlAckChoice.INCREMENT
	}

	if obj.obj.Decrement != nil {
		choices_set += 1
		choice = PatternFlowTcpCtlAckChoice.DECREMENT
	}
	if choices_set == 0 {
		...
		if obj.obj.Choice != nil {
			if obj.Choice() != choice {
				obj.validationErrors = append(obj.validationErrors, "choice not matching with property in PatternFlowTcpCtlAck")
			}
		} else {
			intVal := otg.PatternFlowTcpCtlAck_Choice_Enum_value[string(choice)]
			enumValue := otg.PatternFlowTcpCtlAck_Choice_Enum(intVal)
			obj.obj.Choice = &enumValue
		}
	}
