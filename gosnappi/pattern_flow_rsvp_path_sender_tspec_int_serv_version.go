...
type marshalPatternFlowRSVPPathSenderTspecIntServVersion interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowRSVPPathSenderTspecIntServVersion to raw JSON text
...
func (m *marshalpatternFlowRSVPPathSenderTspecIntServVersion) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
