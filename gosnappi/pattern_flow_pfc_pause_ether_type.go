...
type marshalPatternFlowPfcPauseEtherType interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowPfcPauseEtherType to raw JSON text
...
func (m *marshalpatternFlowPfcPauseEtherType) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowPfcPauseEtherType) setDefault() {
	...
	var choice PatternFlowPfcPauseEtherTypeChoiceEnum

	if obj.obj.Value != nil {
		choices_set += 1
		choice = PatternFlowPfcPauseEtherTypeChoice.VALUE
	}

	if len(obj.obj.Values) > 0 {
		choices_set += 1
		choice = PatternFlowPfcPauseEtherTypeChoice.VALUES
	}

	if obj.obj.Increment != nil {
		choices_set += 1
		choice = PatternFlowPfcPauseEtherTypeChoice.INCREMENT
	}

	if obj.obj.Decrement != nil {
		choices_set += 1
		choice = PatternFlowPfcPauseEtherTypeChoice.DECREMENT
	}
	if choices_set == 0 {
		...
		if obj.obj.Choice != nil {
			if obj.Choice() != choice {
				obj.validationErrors = append(obj.validationErrors, "choice not matching with property in PatternFlowPfcPauseEtherType")
			}
		} else {
			intVal := otg.PatternFlowPfcPauseEtherType_Choice_Enum_value[string(choice)]
			enumValue := otg.PatternFlowPfcPauseEtherType_Choice_Enum(intVal)
			obj.obj.Choice = &enumValue
		}
	}
