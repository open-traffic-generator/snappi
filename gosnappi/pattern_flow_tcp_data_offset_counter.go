...
type marshalPatternFlowTcpDataOffsetCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowTcpDataOffsetCounter to raw JSON text
...
func (m *marshalpatternFlowTcpDataOffsetCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowTcpDataOffsetCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 15 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowTcpDataOffsetCounter.Count <= 15 but Got %d", *obj.obj.Count))
		}
