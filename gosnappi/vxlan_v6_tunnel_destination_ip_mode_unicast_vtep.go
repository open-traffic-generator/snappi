...
type marshalVxlanV6TunnelDestinationIPModeUnicastVtep interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals VxlanV6TunnelDestinationIPModeUnicastVtep to raw JSON text
...
func (m *marshalvxlanV6TunnelDestinationIPModeUnicastVtep) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
