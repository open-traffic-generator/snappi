...
type marshalPatternFlowGtpv1MessageTypeCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowGtpv1MessageTypeCounter to raw JSON text
...
func (m *marshalpatternFlowGtpv1MessageTypeCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowGtpv1MessageTypeCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 255 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowGtpv1MessageTypeCounter.Count <= 255 but Got %d", *obj.obj.Count))
		}
