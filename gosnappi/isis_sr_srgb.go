...
type marshalIsisSRSrgb interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals IsisSRSrgb to raw JSON text
...
func (m *marshalisisSRSrgb) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
	...
	if obj.obj.StartingSid != nil {
		...
		if *obj.obj.StartingSid < 1 || *obj.obj.StartingSid > 4294967295 {
			vObj.validationErrors = append(
				...
				fmt.Sprintf("1 <= IsisSRSrgb.StartingSid <= 4294967295 but Got %d", *obj.obj.StartingSid))
		}
