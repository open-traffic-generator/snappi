...
type marshalPatternFlowGreReserved1Counter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowGreReserved1Counter to raw JSON text
...
func (m *marshalpatternFlowGreReserved1Counter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowGreReserved1Counter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 65535 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowGreReserved1Counter.Count <= 65535 but Got %d", *obj.obj.Count))
		}
