...
type marshalPatternFlowTcpCtlFinCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowTcpCtlFinCounter to raw JSON text
...
func (m *marshalpatternFlowTcpCtlFinCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowTcpCtlFinCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 1 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowTcpCtlFinCounter.Count <= 1 but Got %d", *obj.obj.Count))
		}
