...
type marshalPatternFlowIpv4TosUnusedCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowIpv4TosUnusedCounter to raw JSON text
...
func (m *marshalpatternFlowIpv4TosUnusedCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowIpv4TosUnusedCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 1 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowIpv4TosUnusedCounter.Count <= 1 but Got %d", *obj.obj.Count))
		}
