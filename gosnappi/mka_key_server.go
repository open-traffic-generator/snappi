...
type marshalMkaKeyServer interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals MkaKeyServer to raw JSON text
...
func (m *marshalmkaKeyServer) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *mkaKeyServer) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.RekeyThresholdPn != nil {
		...
		if len(*obj.obj.RekeyThresholdPn) < 1 || len(*obj.obj.RekeyThresholdPn) > 8 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf(
					"1 <= length of MkaKeyServer.RekeyThresholdPn <= 8 but Got %d",
	...
	if obj.obj.RekeyThresholdXpn != nil {
		...
		if len(*obj.obj.RekeyThresholdXpn) < 1 || len(*obj.obj.RekeyThresholdXpn) > 16 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf(
					"1 <= length of MkaKeyServer.RekeyThresholdXpn <= 16 but Got %d",
