...
type marshalPatternFlowGtpv2MessageTypeCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowGtpv2MessageTypeCounter to raw JSON text
...
func (m *marshalpatternFlowGtpv2MessageTypeCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowGtpv2MessageTypeCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 255 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowGtpv2MessageTypeCounter.Count <= 255 but Got %d", *obj.obj.Count))
		}
