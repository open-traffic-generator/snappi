...
type marshalPatternFlowIpv4TosReliabilityCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowIpv4TosReliabilityCounter to raw JSON text
...
func (m *marshalpatternFlowIpv4TosReliabilityCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowIpv4TosReliabilityCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 1 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowIpv4TosReliabilityCounter.Count <= 1 but Got %d", *obj.obj.Count))
		}
