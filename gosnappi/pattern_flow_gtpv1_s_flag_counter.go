...
type marshalPatternFlowGtpv1SFlagCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowGtpv1SFlagCounter to raw JSON text
...
func (m *marshalpatternFlowGtpv1SFlagCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowGtpv1SFlagCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 1 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowGtpv1SFlagCounter.Count <= 1 but Got %d", *obj.obj.Count))
		}
