...
type marshalPatternFlowTcpCtlAckCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowTcpCtlAckCounter to raw JSON text
...
func (m *marshalpatternFlowTcpCtlAckCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowTcpCtlAckCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 1 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowTcpCtlAckCounter.Count <= 1 but Got %d", *obj.obj.Count))
		}
