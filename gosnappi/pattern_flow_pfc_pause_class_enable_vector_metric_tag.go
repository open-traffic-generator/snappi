...
type marshalPatternFlowPfcPauseClassEnableVectorMetricTag interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowPfcPauseClassEnableVectorMetricTag to raw JSON text
...
func (m *marshalpatternFlowPfcPauseClassEnableVectorMetricTag) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
