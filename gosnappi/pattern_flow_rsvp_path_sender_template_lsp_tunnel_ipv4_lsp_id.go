...
type marshalPatternFlowRSVPPathSenderTemplateLspTunnelIpv4LspId interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowRSVPPathSenderTemplateLspTunnelIpv4LspId to raw JSON text
...
func (m *marshalpatternFlowRSVPPathSenderTemplateLspTunnelIpv4LspId) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
