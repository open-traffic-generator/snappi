...
type marshalPatternFlowRSVPPathSenderTspecIntServParameterIdTokenBucketTspecCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowRSVPPathSenderTspecIntServParameterIdTokenBucketTspecCounter to raw JSON text
...
func (m *marshalpatternFlowRSVPPathSenderTspecIntServParameterIdTokenBucketTspecCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowRSVPPathSenderTspecIntServParameterIdTokenBucketTspecCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 255 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowRSVPPathSenderTspecIntServParameterIdTokenBucketTspecCounter.Count <= 255 but Got %d", *obj.obj.Count))
		}
