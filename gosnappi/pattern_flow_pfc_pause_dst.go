...
type marshalPatternFlowPfcPauseDst interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowPfcPauseDst to raw JSON text
...
func (m *marshalpatternFlowPfcPauseDst) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowPfcPauseDst) setDefault() {
	...
	var choice PatternFlowPfcPauseDstChoiceEnum

	if obj.obj.Value != nil {
		choices_set += 1
		choice = PatternFlowPfcPauseDstChoice.VALUE
	}

	if len(obj.obj.Values) > 0 {
		choices_set += 1
		choice = PatternFlowPfcPauseDstChoice.VALUES
	}

	if obj.obj.Increment != nil {
		choices_set += 1
		choice = PatternFlowPfcPauseDstChoice.INCREMENT
	}

	if obj.obj.Decrement != nil {
		choices_set += 1
		choice = PatternFlowPfcPauseDstChoice.DECREMENT
	}
	if choices_set == 0 {
		...
		if obj.obj.Choice != nil {
			if obj.Choice() != choice {
				obj.validationErrors = append(obj.validationErrors, "choice not matching with property in PatternFlowPfcPauseDst")
			}
		} else {
			intVal := otg.PatternFlowPfcPauseDst_Choice_Enum_value[string(choice)]
			enumValue := otg.PatternFlowPfcPauseDst_Choice_Enum(intVal)
			obj.obj.Choice = &enumValue
		}
	}
