...
type marshalPatternFlowPfcPausePauseClass5MetricTag interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowPfcPausePauseClass5MetricTag to raw JSON text
...
func (m *marshalpatternFlowPfcPausePauseClass5MetricTag) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
