...
type marshalResultExtendedCommunityTransitive2OctetAsTypeRouteTarget interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals ResultExtendedCommunityTransitive2OctetAsTypeRouteTarget to raw JSON text
...
func (m *marshalresultExtendedCommunityTransitive2OctetAsTypeRouteTarget) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
