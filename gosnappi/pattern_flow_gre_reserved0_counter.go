...
type marshalPatternFlowGreReserved0Counter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowGreReserved0Counter to raw JSON text
...
func (m *marshalpatternFlowGreReserved0Counter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowGreReserved0Counter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 4095 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowGreReserved0Counter.Count <= 4095 but Got %d", *obj.obj.Count))
		}
