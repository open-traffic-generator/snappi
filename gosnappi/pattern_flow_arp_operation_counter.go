...
type marshalPatternFlowArpOperationCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowArpOperationCounter to raw JSON text
...
func (m *marshalpatternFlowArpOperationCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowArpOperationCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 65535 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowArpOperationCounter.Count <= 65535 but Got %d", *obj.obj.Count))
		}
