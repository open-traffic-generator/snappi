...
type marshalPatternFlowGtpv1SquenceNumberCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowGtpv1SquenceNumberCounter to raw JSON text
...
func (m *marshalpatternFlowGtpv1SquenceNumberCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowGtpv1SquenceNumberCounter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 65535 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowGtpv1SquenceNumberCounter.Count <= 65535 but Got %d", *obj.obj.Count))
		}
