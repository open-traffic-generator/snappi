...
type marshalPatternFlowRSVPPathSessionLspTunnelIpv4ReservedCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowRSVPPathSessionLspTunnelIpv4ReservedCounter to raw JSON text
...
func (m *marshalpatternFlowRSVPPathSessionLspTunnelIpv4ReservedCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
