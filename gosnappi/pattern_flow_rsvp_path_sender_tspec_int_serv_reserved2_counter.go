...
type marshalPatternFlowRSVPPathSenderTspecIntServReserved2Counter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowRSVPPathSenderTspecIntServReserved2Counter to raw JSON text
...
func (m *marshalpatternFlowRSVPPathSenderTspecIntServReserved2Counter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowRSVPPathSenderTspecIntServReserved2Counter) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.Count != nil {
		...
		if *obj.obj.Count > 127 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("0 <= PatternFlowRSVPPathSenderTspecIntServReserved2Counter.Count <= 127 but Got %d", *obj.obj.Count))
		}
