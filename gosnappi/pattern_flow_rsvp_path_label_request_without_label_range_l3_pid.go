...
type marshalPatternFlowRSVPPathLabelRequestWithoutLabelRangeL3Pid interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowRSVPPathLabelRequestWithoutLabelRangeL3Pid to raw JSON text
...
func (m *marshalpatternFlowRSVPPathLabelRequestWithoutLabelRangeL3Pid) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
...
func (obj *patternFlowRSVPPathLabelRequestWithoutLabelRangeL3Pid) setDefault() {
	...
	var choice PatternFlowRSVPPathLabelRequestWithoutLabelRangeL3PidChoiceEnum

	if obj.obj.Value != nil {
		choices_set += 1
		choice = PatternFlowRSVPPathLabelRequestWithoutLabelRangeL3PidChoice.VALUE
	}

	if len(obj.obj.Values) > 0 {
		choices_set += 1
		choice = PatternFlowRSVPPathLabelRequestWithoutLabelRangeL3PidChoice.VALUES
	}

	if obj.obj.Increment != nil {
		choices_set += 1
		choice = PatternFlowRSVPPathLabelRequestWithoutLabelRangeL3PidChoice.INCREMENT
	}

	if obj.obj.Decrement != nil {
		choices_set += 1
		choice = PatternFlowRSVPPathLabelRequestWithoutLabelRangeL3PidChoice.DECREMENT
	}
	if choices_set == 0 {
		...
		if obj.obj.Choice != nil {
			if obj.Choice() != choice {
				obj.validationErrors = append(obj.validationErrors, "choice not matching with property in PatternFlowRSVPPathLabelRequestWithoutLabelRangeL3Pid")
			}
		} else {
			intVal := otg.PatternFlowRSVPPathLabelRequestWithoutLabelRangeL3Pid_Choice_Enum_value[string(choice)]
			enumValue := otg.PatternFlowRSVPPathLabelRequestWithoutLabelRangeL3Pid_Choice_Enum(intVal)
			obj.obj.Choice = &enumValue
		}
	}
