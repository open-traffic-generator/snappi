...
// Once it receives Restarting TLV having RA bit set in a IIH PDU,  time (in seconds), after which IIH PDU, having Restart Tlv with RR bit unset, will be sent. This should result in IIH to be Txed indicating restart is Completed,  not started i.e. RR bit is cleared and hold_timer is reset to normal.
...
func (obj *isisActionProtocolIsisUnplannedRestart) validateObj(vObj *validation, set_default bool) {
	...
	if obj.obj.HoldingTime != nil {
		...
		if *obj.obj.HoldingTime < 1 || *obj.obj.HoldingTime > 4096 {
			vObj.validationErrors = append(
				...
				fmt.Sprintf("1 <= IsisActionProtocolIsisUnplannedRestart.HoldingTime <= 4096 but Got %d", *obj.obj.HoldingTime))
		}
	...
	if obj.obj.RestartAfter != nil {
		...
		if *obj.obj.RestartAfter < 1 || *obj.obj.RestartAfter > 4096 {
			vObj.validationErrors = append(
				vObj.validationErrors,
				fmt.Sprintf("1 <= IsisActionProtocolIsisUnplannedRestart.RestartAfter <= 4096 but Got %d", *obj.obj.RestartAfter))
		}
