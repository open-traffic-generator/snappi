...
type marshalPatternFlowArpTargetProtocolAddrCounter interface {
	...
	ToJson() (string, error)
	// ToJsonRaw marshals PatternFlowArpTargetProtocolAddrCounter to raw JSON text
...
func (m *marshalpatternFlowArpTargetProtocolAddrCounter) ToJsonRaw() (string, error) {
	vErr := m.obj.validateToAndFrom()
	if vErr != nil {
		return "", vErr
	}
