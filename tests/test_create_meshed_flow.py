...
@pytest.mark.skip("models need to be fixed")
def test_create_meshed_flow(api):
    ...
    flow.tx_rx.device.tx_names = [tx.ethernets[-1] for tx in config.devices]
    flow.tx_rx.device.rx_names = [rx.ethernets[-1] for rx in config.devices]
